...
function parseMarkdownToHtml(markdown: string): string {
  ...
  html = html.replace(/^\d+\.[ \t]+[^\n]*(?:\n\d+\.[ \t]+[^\n]*)*/gm, (list) => {
    // The matched marker contains only digits; normalize without numeric precision loss.
    const start = list.slice(0, list.indexOf('.')).replace(/^0+(?=\d)/, '');
    const startAttribute = start === '1' ? '' : ` start="${start}"`;
    const items = list.split('\n').map(line => `<li>${line.replace(/^\d+\.[ \t]+/, '')}</li>`).join('\n');
    return `\n\n<ol class="list-decimal pl-6 space-y-2 my-4"${startAttribute}>${items}</ol>\n\n`;
  });
  html = html.replace(/^- [^\n]*(?:\n- [^\n]*)*/gm, (list) => {
