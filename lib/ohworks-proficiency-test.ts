...
/**
 * Canonicalize a unit for comparison only: trim outer whitespace. Deliberately
 * does NOT case-fold: SI prefixes are case-sensitive (e.g. "m" = milli,
 * "M" = mega), so "mg/L" and "Mg/L" name different quantities and must
 * compare as distinct. Never infers or performs a unit conversion, so
 * "mg/L" and "g/L" also remain distinct. Returns undefined for anything
...
/**
 * Tolerance for inclusive-boundary comparisons, sized to absorb only
 * floating-point representation error from deriving `lower`/`upper` (e.g.
 * assignedValue * (1 +/- percentage / 100)), not to widen the acceptance
 * criterion itself. Scales with the magnitude of the values being compared
 * so it stays negligible relative to any real deviation.
 */
function inclusiveBoundaryEpsilon(...values: number[]): number {
  const scale = values.reduce((max, value) => Math.max(max, Math.abs(value)), 1);
  return scale * Number.EPSILON * 8;
}

function isWithinInclusiveBounds(value: number, lower: number, upper: number): boolean {
  const epsilon = inclusiveBoundaryEpsilon(value, lower, upper);
...
function evaluateSingleRound(round: ProficiencyTestRound, isDuplicate: boolean): ProficiencyRoundEvaluation {
  ...
  // Past this point every input value used below is a validated finite number,
  // but values *derived* from them (deviation, percentDeviation, and the
  // percentage-of-assigned bounds) can still overflow to +/-Infinity or NaN,
  // so they are guarded before being trusted for classification.
  ...
  if (
    ...
    !Number.isFinite(safeLower) ||
    !Number.isFinite(safeUpper) ||
    (percentDeviation !== undefined && !Number.isFinite(percentDeviation))
  ) {
    return {
      ...
      deviation: undefined,
      percentDeviation: undefined,
      reasons: [{ code: 'derived-result-overflow' }],
    };
  }

  const classification: ProficiencyRoundClassification = isWithinInclusiveBounds(safeReported, safeLower, safeUpper)
    ? 'acceptable'
    : 'unacceptable';
 ...
 * percentage criterion (non-finite or negative), a duplicate round
 * identifier, or a derived result (deviation, percentDeviation, or a
 * percentage-of-assigned bound) that overflows to a non-finite number even
 * though every input was finite, all classify the affected round(s) as
 * 'not_evaluated' rather than being guessed at. 'not_evaluated' rounds are
 * excluded from the per-analyte consecutive-unacceptable streak (they
 * neither extend nor reset it), so bad data cannot be used to mask or
 * manufacture a streak. A structurally unusable batch or threshold throws
 * ProficiencyTestInputError instead of guessing at a per-round evaluation.
 */
