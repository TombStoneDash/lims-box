...
export type SyntheticLabDayStep = 'effective-definition' | 'unit-conversion' | 'reference-range' | 'delta-check' | 'qc-rules' | 'volume-ledger' | 'autoverification' | 'turnaround-time';
...
export function runSyntheticLabDay(input: SyntheticLabDayInput): SyntheticLabDayReport {
  ...
  return { results, totals: { released, held: results.length - released, heldByReason, tatBreaches: new Set(results.filter(r => r.turnaround?.status === 'breached').map(r => r.specimenId)).size } };
}
