...
def _speak_piper(text: str) -> None:
    ...
    stdout, _ = _communicate_checked(proc, text.encode())
...
def _communicate_checked(proc: subprocess.Popen, data: bytes):
    """Check process completion and reap timed-out children before falling back."""
    try:
        result = proc.communicate(input=data, timeout=30)
