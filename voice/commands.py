...
def _handle_print_label(args: tuple, client: SenaiteClient) -> str:
    ...
    if not sample.get("api_url") or not sample.get("uid"):
        return f"Label request not sent for sample {sample_id}: required sample metadata is missing."
    ...
    try:
        ...
        return f"Label request submitted to SENAITE for sample {sample_id}. Printing is not confirmed."
    except Exception as e:
        logger.warning(f"Label print API call failed: {e}")
        return f"Label request failed for sample {sample_id}. Printing is not confirmed."
