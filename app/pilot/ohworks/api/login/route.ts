...
export async function POST(request: NextRequest) {
  ...
  if (!principal) return NextResponse.redirect(new URL('/pilot/ohworks/login?error=1', request.url), 303);
  const response = NextResponse.redirect(new URL('/pilot/ohworks', request.url), 303);
